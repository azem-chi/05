...
function showOnboarding() {
  ...
  // أول شاشة: إذا اللغة محددة → اسأل "لديك حساب؟"
  // إذا لا → اختر اللغة أولاً ثم نفس السؤال
  obCurrentStep = langAlreadySet ? 'returning' : 'lang';
...
function obNext() {
  ...
  if (step === 'info') {
    ...
    if (h > 0) S.user.height = h;
    if (a > 0) S.user.age    = a;
    if (nEl?.value.trim()) S.user.name = nEl.value.trim();
  ...
  if (step === 'privacy') { obFinish(); return; }
  obFinish();
}
...
function renderObStep() {
  ...
  const hideFooter = ['lang','welcome'].includes(step);
  if (footerEl)  footerEl.style.display  = hideFooter ? 'none' : 'flex';
  if (skipBtnEl) skipBtnEl.style.display = step === 'auth' ? 'block' : 'none';
  if (nextBtn) {
    const last = ['auth','privacy'].includes(step);
    nextBtn.textContent = last
      ? _t('🚀 ابدأ رحلتك','🚀 Start','🚀 Commencer')
      : _t('التالي ←','Next →','Suivant →');
  }

  const VISIBLE_DOTS = ['welcome','goal','body','level','days','auth'];
  const dotsEl = document.getElementById('ob-dots');
  // returning/login لا تعرض dots
  if (['returning','login'].includes(step)) {
    ...
    el.innerHTML = `
      <div style="min-height:100vh;display:flex;flex-direction:column;
        ...
        <div style="text-align:center;margin-bottom:16px;">
          ...
          <div style="font-size:24px;font-weight:900;color:var(--txt);margin-bottom:8px;">
            ${_t('مرحباً بعودتك!','Welcome back!','Content de te revoir!')}
          </div>
          <div style="font-size:13px;color:var(--dim);line-height:1.6;">
            ${_t(
              'سجّل دخولك لاسترجاع كل بياناتك وتقدمك فوراً',
              'Sign in to restore all your data & progress instantly',
              'Connecte-toi pour récupérer toutes tes données'
            )}
          </div>
        </div>

        <!-- Google Sign In -->
        <button id="ob-google-btn" onclick="obReturnSignIn()"
          style="width:100%;padding:18px;border-radius:18px;
          background:rgba(66,133,244,.12);border:2px solid rgba(66,133,244,.4);
          color:#4285f4;font-family:'Cairo',sans-serif;font-size:16px;font-weight:800;
          cursor:pointer;display:flex;align-items:center;justify-content:center;gap:12px;">
        ...
        <div style="text-align:center;font-size:12px;color:var(--dim);
          padding:12px;border-radius:12px;
          background:rgba(212,168,67,.06);border:1px solid rgba(212,168,67,.12);">
          ${_t(
            '⚡ سيتم تحميل بياناتك وتقدمك فوراً بعد تسجيل الدخول — بدون إعادة إدخال أي معلومة',
            '⚡ Your data & progress will load instantly — no re-entry needed',
            '⚡ Vos données se chargeront instantanément'
          )}
        </div>

        <button onclick="obGoToStep('returning')"
          style="padding:12px;border-radius:14px;background:transparent;
  ...
  // ── LANG ──
  if (step === 'lang') {
    const langs = [{code:'ar',flag:'🇩🇿',name:'العربية',sub:'Arabic'},{code:'en',flag:'🇬🇧',name:'English',sub:'Anglais'},{code:'fr',flag:'🇫🇷',name:'Français',sub:'French'}];
    el.innerHTML = `<div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:40px 24px;">
      ...
      <div style="display:flex;flex-direction:column;gap:12px;width:100%;max-width:320px;">
        ${langs.map(l => { const active=(S.lang||'ar')===l.code; return `<button onclick="obSelectLang('${l.code}')" style="display:flex;align-items:center;gap:16px;padding:18px 22px;border-radius:18px;background:${active?'rgba(212,168,67,.15)':'var(--card)'};border:2px solid ${active?'var(--gold)':'var(--border)'};cursor:pointer;width:100%;"><span style="font-size:32px;">${l.flag}</span><div style="flex:1;"><div style="font-size:17px;font-weight:900;color:${active?'var(--gold)':'var(--txt)'};">${l.name}</div><div style="font-size:11px;color:var(--dim);">${l.sub}</div></div>${active?'<span style="color:var(--gold);font-size:20px;">✓</span>':''}</button>`; }).join('')}
      </div></div>`;
    return;
  }
  ...
  if (step === 'welcome') {
    const feats = _ar
      ? [{icon:'🤖',t:'مدرب ذكي',s:'يعرف وزنك وهدفك ويتذكرك'},{icon:'📊',t:'تتبع التقدم',s:'رسوم بيانية وإحصاء ذكي'},{icon:'🔥',t:'برنامج مخصص',s:'يتكيف مع أدائك الفعلي'},{icon:'🏆',t:'مستويات وشارات',s:'10 مستويات + تحديات أسبوعية'}]
      : _en
      ? [{icon:'🤖',t:'AI Coach',s:'Knows your goals & adapts'},{icon:'📊',t:'Track Progress',s:'Charts & smart analytics'},{icon:'🔥',t:'Custom Program',s:'Adapts to your performance'},{icon:'🏆',t:'Levels & Badges',s:'10 levels + weekly challenges'}]
      : [{icon:'🤖',t:'Coach IA',s:"Connaît tes objectifs"},{icon:'📊',t:'Suivi progrès',s:'Graphiques et statistiques'},{icon:'🔥',t:'Programme perso',s:"S'adapte à tes performances"},{icon:'🏆',t:'Niveaux & badges',s:'10 niveaux + défis hebdo'}];
    el.innerHTML = `<div style="min-height:100vh;display:flex;flex-direction:column;padding:48px 24px 24px;">
      <div style="text-align:center;margin-bottom:32px;">
        <div style="font-size:56px;margin-bottom:12px;">💪</div>
        <div style="font-size:26px;font-weight:900;color:var(--txt);margin-bottom:8px;">${_t('مرحباً بك في AZEM','Welcome to AZEM','Bienvenue dans AZEM')}</div>
        <div style="font-size:13px;color:var(--dim);line-height:1.7;">${_t('تطبيق اللياقة العربي المدعوم بالذكاء الاصطناعي','The Arabic AI-powered fitness app','App fitness arabe propulsée par IA')}</div>
      </div>
      <div style="display:flex;flex-direction:column;gap:12px;margin-bottom:36px;">
        ${feats.map(f=>`<div style="display:flex;align-items:center;gap:14px;padding:14px 16px;background:var(--card);border:1px solid var(--border);border-radius:14px;"><div style="width:44px;height:44px;border-radius:12px;background:rgba(212,168,67,.1);border:1.5px solid rgba(212,168,67,.2);display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0;">${f.icon}</div><div><div style="font-size:14px;font-weight:800;color:var(--txt);">${f.t}</div><div style="font-size:12px;color:var(--dim);margin-top:2px;">${f.s}</div></div></div>`).join('')}
      </div>
      <button onclick="obGoToStep('goal')" style="width:100%;padding:18px;border-radius:18px;background:linear-gradient(135deg,var(--gl),var(--gd));border:none;color:var(--night);font-family:'Cairo',sans-serif;font-size:17px;font-weight:900;cursor:pointer;box-shadow:0 6px 24px var(--glow);">${_t('ابدأ رحلتك ←','Start your journey →','Commencer →')}</button>
      <div style="text-align:center;margin-top:10px;font-size:11px;color:var(--dim);">${_t('يستغرق الإعداد أقل من دقيقة','Setup takes less than 1 minute','Configuration en moins de 1 minute')}</div>
    </div>`;
    return;
  }
  ...
  if (step === 'goal') {
    const goals = _ar
      ? [{id:'burn',icon:'🔥',t:'حرق الدهون',s:'أخسر وزناً وأحصل على جسم ممشوق',c:'-300 كال'},{id:'muscle',icon:'💪',t:'بناء العضلات',s:'أزيد الكتلة العضلية والقوة',c:'+300 كال'},{id:'fitness',icon:'⚡',t:'تحسين اللياقة',s:'أتحسن في التحمل والأداء العام',c:'= كال'},{id:'health',icon:'❤️',t:'الصحة العامة',s:'أعيش بشكل أكثر نشاطاً وصحة',c:'متوازن'}]
      : _en
      ? [{id:'burn',icon:'🔥',t:'Fat Burn',s:'Lose weight & get lean',c:'-300 cal'},{id:'muscle',icon:'💪',t:'Build Muscle',s:'Increase mass & strength',c:'+300 cal'},{id:'fitness',icon:'⚡',t:'Improve Fitness',s:'Better endurance & performance',c:'= cal'},{id:'health',icon:'❤️',t:'General Health',s:'Live more active & healthy',c:'balanced'}]
      : [{id:'burn',icon:'🔥',t:'Brûler graisses',s:'Perdre du poids et mincir',c:'-300 cal'},{id:'muscle',icon:'💪',t:'Musculation',s:'Augmenter la masse musculaire',c:'+300 cal'},{id:'fitness',icon:'⚡',t:'Forme physique',s:"Améliorer l'endurance",c:'= cal'},{id:'health',icon:'❤️',t:'Santé générale',s:'Vivre plus actif et en santé',c:'équilibré'}];
  ...
  if (step === 'body') {
    ...
    el.innerHTML = `<div style="padding:32px 24px;">
      ...
      <div style="margin-bottom:14px;">
        ...
        <input id="ob-name-inp2" type="text" value="${_escHtml(S.user?.name||'')}" placeholder="${_t('اسمك...','Your name...','Ton prénom...')}" style="width:100%;padding:14px 16px;border-radius:14px;background:var(--card);border:1.5px solid var(--border);color:var(--txt);font-family:'Cairo',sans-serif;font-size:15px;-webkit-appearance:none;appearance:none;">
      </div>
      <div style="margin-bottom:14px;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:14px;">
        ${[{id:'ob-age-inp',l:_t('العمر','Age','Âge'),u:_t('سنة','yr','ans'),v:S.user?.age||'',p:'25'},{id:'ob-weight-inp',l:_t('الوزن','Weight','Poids'),u:'kg',v:S.user?.weight||'',p:'70'},{id:'ob-height-inp',l:_t('الطول','Height','Taille'),u:'cm',v:S.user?.height||'',p:'170'}].map(f=>`<div><div style="font-size:11px;font-weight:700;color:var(--dim);margin-bottom:6px;text-align:center;">${f.l} <span style="opacity:.6;">(${f.u})</span></div><input id="${f.id}" type="number" value="${f.v}" placeholder="${f.p}" style="width:100%;padding:14px 8px;border-radius:14px;text-align:center;background:var(--card);border:1.5px solid var(--border);color:var(--txt);font-family:'Cairo',sans-serif;font-size:16px;font-weight:700;-webkit-appearance:none;appearance:none;"></div>`).join('')}
      </div>
  ...
  if (step === 'level') {
    const lvls = _ar
      ? [{id:'beginner',icon:'🌱',t:'مبتدئ',s:'لم أتمرن منذ فترة أو لأول مرة',d:20},{id:'intermediate',icon:'⚡',t:'متوسط',s:'أتمرن أحياناً، أعرف الأساسيات',d:25},{id:'advanced',icon:'🔥',t:'متقدم',s:'أتمرن بانتظام ولديّ تجربة',d:30}]
      : _en
      ? [{id:'beginner',icon:'🌱',t:'Beginner',s:"Haven't trained recently",d:20},{id:'intermediate',icon:'⚡',t:'Intermediate',s:'Train sometimes, know basics',d:25},{id:'advanced',icon:'🔥',t:'Advanced',s:'Train regularly, experienced',d:30}]
      : [{id:'beginner',icon:'🌱',t:'Débutant',s:'Pas entraîné depuis longtemps',d:20},{id:'intermediate',icon:'⚡',t:'Intermédiaire',s:'Entraîné parfois, connaît bases',d:25},{id:'advanced',icon:'🔥',t:'Avancé',s:'Entraîné régulièrement',d:30}];
  ...
  // ── AUTH ──
  if (step === 'auth') {
    el.innerHTML = `<div style="min-height:80vh;display:flex;flex-direction:column;justify-content:center;padding:32px 24px;gap:14px;">
      <div style="text-align:center;margin-bottom:8px;">
        <div style="font-size:48px;margin-bottom:10px;">☁️</div>
        <div style="font-size:22px;font-weight:900;color:var(--txt);margin-bottom:8px;">${_t('احفظ تقدمك في السحابة','Save your progress','Sauvegardez vos progrès')}</div>
        <div style="font-size:13px;color:var(--dim);line-height:1.6;">${_t('سجّل دخولك لمزامنة بياناتك على أجهزة متعددة','Sign in to sync across devices & never lose data','Connectez-vous pour synchroniser')}</div>
      </div>
      <button id="ob-google-btn" onclick="obFirebaseGoogleSignIn()" style="width:100%;padding:16px;border-radius:16px;background:rgba(66,133,244,.12);border:2px solid rgba(66,133,244,.4);color:#4285f4;font-family:'Cairo',sans-serif;font-size:15px;font-weight:800;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:10px;">
      ...
      <button onclick="obFinish()" style="width:100%;padding:14px;border-radius:14px;background:transparent;border:1.5px solid var(--border);color:var(--dim);font-family:'Cairo',sans-serif;font-size:14px;cursor:pointer;">${_t('تخطي — ابدأ بدون حساب','Skip — start without account','Ignorer — commencer sans compte')}</button>
      <div style="text-align:center;font-size:11px;color:var(--dim);opacity:.6;">${_t('بياناتك محفوظة محلياً حتى بدون حساب','Your data is saved locally even without account','Données sauvegardées localement')}</div>
    </div>`;
    return;
  }
  ...
  if (step === 'privacy') {
    el.innerHTML = `<div style="padding:32px 24px;">
      <div style="font-size:22px;font-weight:900;color:var(--txt);margin-bottom:16px;">${_t('الخصوصية','Privacy','Confidentialité')}</div>
      <div style="font-size:13px;color:var(--dim);line-height:1.8;margin-bottom:24px;">${_t('بياناتك محفوظة على جهازك فقط. لا نشارك معلوماتك مع أي طرف ثالث.','Your data is stored locally. We never share your info.','Vos données restent sur votre appareil.')}</div>
      <label style="display:flex;align-items:center;gap:12px;cursor:pointer;margin-bottom:24px;">
        <input type="checkbox" id="ob-privacy-chk" onchange="window._privacyAccepted=this.checked" style="width:20px;height:20px;accent-color:var(--gold);">
        <span style="font-size:14px;color:var(--txt);">${_t('أوافق على الشروط وسياسة الخصوصية','I agree to Terms & Privacy Policy','J\'accepte les Conditions')}</span>
      </label></div>`;
    return;
  }
}
